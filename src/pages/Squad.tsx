...
export default function Squad() {
  ...
  const roles = ['All', 'Principal & Chief Mentor', 'Captain', 'Vice Captain', 'Associate VC', 'Quality Director', 'Player'];
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-8">
          ...
          <div className="flex flex-wrap gap-2">
            {roles.map(role => (
              <button key={role} onClick={() => setFilterRole(role)} className={filterRole === role ? 'bg-orange-700 text-white px-4 py-2 rounded-lg font-medium' : 'bg-gray-100 text-gray-700 hover:bg-gray-200 px-4 py-2 rounded-lg font-medium'}>
                {role}
              </button>
            ))}
          </div>
          ...
          {filteredPlayers.map((player, index) => (
            <div key={index} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition">
              <div className="bg-gradient-to-r from-orange-700 to-orange-600 p-6 text-white">
                ...
                <p className="text-orange-100 text-sm">{player.position}</p>
                <div className="flex items-center gap-2 mt-2">
                  <span className="bg-blue-900 px-3 py-1 rounded-full text-xs font-semibold">{player.role}</span>
                  {player.battingHand && <span className="bg-blue-900 px-3 py-1 rounded-full text-xs">{player.battingHand} Hand</span>}
                </div>
              </div>
              <div className="p-6">
        ...
        <div className="mt-8 bg-orange-800 text-white p-6 rounded-lg text-center">
